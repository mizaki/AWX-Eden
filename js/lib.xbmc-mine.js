...
(function($) {
	...
	$.extend(xbmc, {
		...
		addSongToPlaylist: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "AudioPlaylist.Add", "params": {"item": {"songid": ' + settings.songid + '}}, "id": 1}',
				settings.onSuccess,
				settings.onError,
				null,
				settings.async
		...
		addAudioFileToPlaylist: function(options) {
			...
			xbmc.sendCommand(
				//'{"jsonrpc": "2.0", "method": "AudioPlaylist.Add", "params": { "item": {"file": "' + settings.file.replace(/\\/g, "\\\\") + '"} }, "id": 1}',
				'{"jsonrpc": "2.0", "method": "AudioPlaylist.Add", "params": { "item": {"file": "' + settings.file.replace(/\\/g, "\\\\") + '"} }, "id": 1}',
				settings.onSuccess,
				settings.onError,
				null,
		...
		addAlbumToPlaylist: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "AudioLibrary.GetSongs", "params": {"albumid": ' + settings.albumid + ', "properties": ["track"]}, "id": 1}',
		...
		playAudio: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "AudioPlaylist.Play", "params": {"item": ' + settings.item + '}, "id": 1}',
				settings.onSuccess,
				function(response) {
		...
		getAudioPlaylist: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "AudioPlaylist.GetItems", "params": { "properties": ["title", "album", "artist", "duration"] }, "id": 1}',
		...
		playVideo: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "VideoPlaylist.Play", "params": {"item": ' + settings.item + '}, "id": 1}',
				settings.onSuccess,
				function(response) {
					settings.onError(mkf.lang.get('message_failed_play'));
				}
		...
		/*
			...
			var args = new myobject();
			args.movieid = settings.movieid;
		*/
			xbmc.sendCommand(
				//'{"jsonrpc": "2.0", "method": "VideoPlaylist.Add", "params": {"movieid": ' + settings.movieid + '}, "id": 1}',
				'{"jsonrpc": "2.0", "method": "VideoPlaylist.Add", "params": {"item": {"movieid": ' + settings.movieid + '}}, "id": 1}',
				settings.onSuccess,
				settings.onError,
				null,
				settings.async
		...
		getMovies: function(options) {
			...
			xbmc.sendCommand(
				//'{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": { "start": 0, "properties" : ["genre", "director", "plot", "title", "originaltitle", "runtime", "year", "rating"], "sort": { "order": "ascending", "method": "label" } }, "id": 1}',
				'{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": {"properties" : ["genre", "director", "plot", "title", "originaltitle", "runtime", "year", "rating", "thumbnail"], "sort": { "order": "ascending", "method": "label" } }, "id": 1}',
				function(response) {
					settings.onSuccess(response.result);
				},
				settings.onError
		...
		getRecentMovies: function(options) {
			var settings = {
				onSuccess: null,
				onError: null,
			};
			...
			xbmc.sendCommand(
				'{ "jsonrpc": "2.0", "method": "VideoLibrary.GetRecentlyAddedMovies", "params": { "limits" : {"start" : 0, "end" : 9}, "properties" : ["genre", "director", "plot", "title", "originaltitle", "runtime", "year", "rating", "thumbnail"], "sort" : { "order" : "ascending", "method" : "label" } }, "id" : 1}',
				function(response) {
					settings.onSuccess(response.result);
				},
				settings.onError
		...
		addEpisodeToPlaylist: function(options) {
			...
			xbmc.sendCommand(
				...
				'{"jsonrpc": "2.0", "method": "VideoPlaylist.Add", "params": {"item": {"episodeid": ' + settings.episodeid + '}}, "id": 1}',
				settings.onSuccess,
				settings.onError,
				null,
				settings.async
		...
		getTvShows: function(options) {
			...
			xbmc.sendCommand(
				//'{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShows", "params": { "start": 0, "properties": ["genre", "director", "plot", "title", "originaltitle", "runtime", "year", "rating"] }, "id": 1}',
				'{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShows", "params": { "properties": ["genre", "plot", "title", "originaltitle", "year", "rating", "thumbnail"] }, "id": 1}',
				function(response) {
					settings.onSuccess(response.result);
				},
				settings.onError
		...
		getSeasons: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "VideoLibrary.GetSeasons", "params": { "tvshowid": ' + settings.tvshowid + ', "properties": ["season"]}, "id": 1}',
				function(response) {
					settings.onSuccess(response.result);
				},
				settings.onError
		...
		getEpisodes: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodes", "params": { "tvshowid": ' + settings.tvshowid + ', "season" : ' + settings.season + ', "properties": ["episode"], "sort": { "order": "ascending", "method": "episode" } }, "id": 1}',
				function(response) {
					settings.onSuccess(response.result);
				},
				settings.onError
		...
		getRecentEpisodes: function(options) {
			var settings = {
			//	tvshowid: 0,
			//	season: 0,
				onSuccess: null,
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "VideoLibrary.GetRecentlyAddedEpisodes", "params": { "sort" : { "order" : "ascending" }, "properties": ["episode", "showtitle", "season", "thumbnail"], "limits" : { "start" : 0, "end" : 9 }, "id": 1}',
				function(response) {
					settings.onSuccess(response.result);
				},
				settings.onError
				...
				if ((this.volumeChangedListener &&
					...
					if (activePlayer != 'none') {
						...
						if (activePlayer == 'audio') {
							request = '{"jsonrpc": "2.0", "method": "AudioPlaylist.GetItems", "params": { "properties": ["title", "album", "artist", "duration"] }, "id": 1}';

						} else if (activePlayer == 'video') {
							request = '{"jsonrpc": "2.0", "method": "VideoPlaylist.GetItems", "params": { "properties": ["title", "season", "episode", "duration", "showtitle"] }, "id": 1}';
						}
