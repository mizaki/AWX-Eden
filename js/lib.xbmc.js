...
(function($) {
	...
	$.extend(xbmc, {
				...
				xbmc.sendCommand(
					...
					function (response) {
						if (response.result[0]) {
							//var playerResult = response.result[0].type;
							//var currentPlayer = '';
							var playeridis = response.result[0].playerid;
							...
							//if (playerResult.audio) 		playeridis = 0;
							//else if (playerResult.video)	playeridis = 1;
							//else if (playerResult.picture)	playeridis = 2;
							...
							xbmc.sendCommand(
								...
								'{"jsonrpc": "2.0", "method": "Player.' + commands[settings.type] + '", "params": { "playerid": ' + playeridis + ' }, "id": 1}',
								settings.onSuccess,
								settings.onError
							);
							//}
		...
		addAudioFileToPlaylist: function(options) {
			...
			xbmc.sendCommand(
				//'{"jsonrpc": "2.0", "method": "Playlist.Add", "params": { "item": {"file": "' + settings.file.replace(/\\/g, "\\\\") + '"} }, "id": 1}',
		...
		playAudio: function(options) {
			...
			xbmc.sendCommand(
				// '{"jsonrpc": "2.0", "method": "Player.Open", "params": {"item": ' + settings.item + '}, "id": 1}',
		...
		*/
		...
		getVideoPlaylist: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "Playlist.GetItems", "params": { "playlistid": 1}, "id": 1}',
